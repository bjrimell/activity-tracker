...
const ActivityList = () => {
    ...
    const reduceFrequency = async (activityId) => {
        try {
            ...
            const response = await axios.put(`https://7w5y9pjq74.execute-api.us-east-1.amazonaws.com/Prod/activity/${activityId}`, {
                ...
                Frequency: 1 // This can be replaced by current frequency - 1
            });
            ...
            {Array.isArray(activities) && activities.length > 0 ? (
                activities.map(activity => (
                    <div key={activity.ActivityId}>
                        ...
                        <button onClick={() => reduceFrequency(activity.ActivityId)}>Reduce Frequency</button>
                    </div>
                ))
            ) : (
